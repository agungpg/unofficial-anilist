...
const AnimeDetailLoading = () => {
  ...
  return <AnimeDetailContainer>
    ...
    <DetailBodyInfoWrapper>
      ...
      <InfoWrapper>
        {infoList.map(item => (<Skeleton key={item.key} style={{ width: '100%', height: '24px', margin: '0 12px 0 0' }} />))}
      </InfoWrapper>
      <GenresWrapper>
        {[1,2,3,4,5].map(item => (<Skeleton key={item*Math.random()} style={{ width: '40px', height: '24px', margin: '4px' }} />))}
      </GenresWrapper>  
      <div>
      {[1,2,3].map(item => <DescriptionLineLoading key={item*Math.random()} />)}
      </div>  
    </DetailBodyInfoWrapper>
